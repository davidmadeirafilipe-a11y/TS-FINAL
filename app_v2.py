...
st.set_page_config(page_title="TES Radar (Excel v2)", layout="wide")
st.title("Thermal Energy Storage — Radar from Excel (v2)")

st.markdown(
    "Upload your workbook (.xlsx) with the **DATA** and **RANK** sheets from your tool. "
    "The app applies your **Both** logic to select materials by temperature and draws a radar with the 1–5 criteria ranks.\n\n"
    "**Both logic**\n"
...
def parse_range(cell):
    ...
    if s.startswith("<="):
        try:
            ...
            return ("empty", np.nan, np.nan)
    # interval a-b (supports open bounds like '-500' or '300-')
    if "-" in s:
